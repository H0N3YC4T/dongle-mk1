...
static void tap_sens_up(int cell) { ARG_UNUSED(cell); prospector_touchpad_sens_step(+1); build_settings(); }
static void tap_sens_dn(int cell) { ARG_UNUSED(cell); prospector_touchpad_sens_step(-1); build_settings(); }
static void tap_bright_up(int cell) { ARG_UNUSED(cell); prospector_brightness_step(+BRIGHTNESS_STEP); build_settings(); }
static void tap_bright_dn(int cell) { ARG_UNUSED(cell); prospector_brightness_step(-BRIGHTNESS_STEP); build_settings(); }
...
static void build_settings(void)
{
  ...
  if (sn >= 0) {
      ...
      if (cur_view_btns[3]) lv_obj_set_style_border_color(cur_view_btns[3], lv_color_hex(sn <= 0 ? COLOR_GREY : COLOR_YELLOW), LV_PART_MAIN);
      if (cur_view_btns[6]) {
  ...
  if (cur_view_btns[5]) lv_obj_set_style_border_color(cur_view_btns[5], lv_color_hex(br <= SETTINGS_BRIGHT_MIN ? COLOR_GREY : COLOR_YELLOW), LV_PART_MAIN);
  if (cur_view_btns[7]) {
