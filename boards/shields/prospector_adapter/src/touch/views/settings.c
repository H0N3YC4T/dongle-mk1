...
static void tap_sens(int delta)
{
  prospector_touchpad_sens_step(delta);
  touch_prefs_save();
  build_settings();
}
static void tap_bright(int delta)
{
  prospector_brightness_step(delta);
  touch_prefs_save();
  build_settings();
}
static void tap_rotate(int cell)
{
  ARG_UNUSED(cell);
  ui_rot = (ui_rot + 1) & 3;
...
static void build_settings(void)
{
  ...
  if (sn >= 0)
  {
    if (cur_view_btns[SET_BTN_SENS_PLUS])
      lv_obj_set_style_border_color(cur_view_btns[SET_BTN_SENS_PLUS], lv_color_hex(theme_color(sn >= SETTINGS_SENS_MAX ? THEME_MUTED : THEME_ACCEPT)), LV_PART_MAIN);
    if (cur_view_btns[SET_BTN_SENS_MINUS])
  ...
  {
    if (cur_view_btns[SET_BTN_SENS_PLUS])
      lv_obj_add_flag(cur_view_btns[SET_BTN_SENS_PLUS], LV_OBJ_FLAG_HIDDEN);
    if (cur_view_btns[SET_BTN_SENS_MINUS])
      lv_obj_add_flag(cur_view_btns[SET_BTN_SENS_MINUS], LV_OBJ_FLAG_HIDDEN);
    if (cur_view_btns[SET_BTN_SENS_READOUT])
      lv_obj_add_flag(cur_view_btns[SET_BTN_SENS_READOUT], LV_OBJ_FLAG_HIDDEN);
  }

  if (cur_view_btns[SET_BTN_BRIGHT_PLUS])
    lv_obj_set_style_border_color(cur_view_btns[SET_BTN_BRIGHT_PLUS], lv_color_hex(theme_color(br >= SETTINGS_BRIGHT_MAX ? THEME_MUTED : THEME_ACCEPT)), LV_PART_MAIN);
  if (cur_view_btns[SET_BTN_BRIGHT_MINUS])
...
static void hold_theme(int cell)
{
  if (cell == 7)
  {
    theme_reset_classic();
