...
static const struct page_cell *find_cell_at(int cell)
{
  ...
  for (int i = 0; active_cells[i].row_span != 0 || active_cells[i].col_span != 0; i++)
  {
    ...
    int row_span = pc->row_span > 0 ? pc->row_span : 1;
    int col_span = pc->col_span > 0 ? pc->col_span : 1;
    if (row >= pc->row && row < pc->row + row_span && col >= pc->col && col < pc->col + col_span)
    {
      return pc;
    }
  }
...
void tap_declarative(int cell)
{
  ...
  if (pc != NULL)
  {
    ...
    case ACT_CUSTOM_VAL:
      if (pc->arg.custom.cb)
        pc->arg.custom.cb(pc->arg.custom.val);
      break;
    case ACT_CUSTOM:
      if (pc->arg.func)
        pc->arg.func(cell);
...
void build_view(const struct view_def *d)
{
  ...
  if (active_cells != NULL)
  {
    ...
    for (int i = 0; active_cells[i].row_span != 0 || active_cells[i].col_span != 0; i++)
    {
      int row = active_cells[i].row + (active_cells[i].row_span > 0 ? active_cells[i].row_span : 1);
      int col = active_cells[i].col + (active_cells[i].col_span > 0 ? active_cells[i].col_span : 1);
      if (row > max_row)
        max_row = row;
      if (col > max_col)
        max_col = col;
    ...
    for (int i = 0; active_cells[i].row_span != 0 || active_cells[i].col_span != 0; i++)
    {
      ...
      if (i < 32)
      {
        if (pc->icon)
        {
          cur_view_btns[i] = draw_cell_icon_ext(pc->row, pc->col, col_span, row_span, pc->icon, pc->label, pc->color);
        }
        else if (pc->label)
        {
          cur_view_btns[i] = draw_cell_ext(pc->row, pc->col, col_span, row_span, pc->label, pc->color, false);
        }
      }
    }
  }
  ...
  if (pending_mods)
  {
    ...
    if (frame != NULL)
    {
      ...
      lv_obj_set_style_border_color(frame, lv_color_hex(theme_color(pending_mod_role)), LV_PART_MAIN);
